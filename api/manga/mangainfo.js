...
const meta = {
  name: "Manga Info",
  desc: "Fetches detailed manga information from MyAnimeList based on a provided URL",
  method: "get",
  category: "manga",
  path: "/mangainfo?url="
};
...
async function onStart({ res, req }) {
  ...
  if (!mangaUrl) throw new Error("Please provide a manga URL using the 'url' query parameter.");
  ...
  const information = {
    ...
    serialization: $("span:contains('Serialization:')").nextAll("a").map((_, el) => $(el).text().trim()).get().join(', ') || 'None',
    authors: $("span:contains('Authors:')").nextAll("a").map((_, el) => $(el).text().trim()).get().join(', ') || 'Unknown',
  };
  ...
  res.json({ status: true, title, synopsis, background, alternativeTitles, information, statistics, externalLinks, imageUrl, link: mangaUrl });
}
