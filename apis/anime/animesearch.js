...
const meta = {
  name: "Anime Search",
  desc: "Searches for anime on MyAnimeList based on a query parameter",
  method: ["get", "post"],
  category: "anime",
...
async function onStart({ res, req }) {
  const searchQuery = req.method === 'POST' ? req.body.query : req.query.query;
  ...
  $('table tbody tr').each((_, element) => {
    ...
    if (title && link) animeList.push({ title, description, type, episodes, score, imageUrl, link });
  });
